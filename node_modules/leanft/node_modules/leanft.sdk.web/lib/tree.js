...
/**
 * A set of xpath values to help the <see cref="ITreeView"/> object perform Select, Expand, and Collapse operations in cases where the default implementation cannot find the element needed.
 * @class TreeTO
 * @memberof Web
 * @constructor
 ...
 * @mixes Web.TreeBehaviour
 */
function TreeTO(session,description) {
    var treeTO = new WebBase.WebBaseTestObject("WebTree", session, description);

    ObjectUtils.extend(treeTO, ClickBehavior);
    ObjectUtils.extend(treeTO, DragAndDropBehavior);
    ObjectUtils.extend(treeTO, WebElement.WebElementTOBehavior);
    ObjectUtils.extend(treeTO, TreeBehaviour);
...
var TreeBehaviour = {
     ...
     * If the xpath parameter is passed, the specified node in this tree is selected with both the node path and XPath.
     ...
     * @param {string} [selector] The xpath used for finding the internal element used for selecting a tree node.
     * @param {string} [expander] The xpath value used to find the internal tree element used to expand and collapse a tree node.
     * @returns {Promise<void>} a promise that is fulfilled after the operation is performed.
     */
    select: function(nodePath, selector, expander){
        selector = selector === undefined ? null : selector;
        expander = expander === undefined ? null : expander;
        return this._executeWithEvents(arguments, "Select", nodePath, selector, expander);
    },
    /**
     * Collapses this tree based on the specified path.
     * If the xpath parameter is passed, the specified node in this tree is collapsed with both the node path and XPath.
     * @param {string} [nodePath] The path to the tree node to collapse. <br/>
     * Note: You can use the buildNodePath function to create the node path.
     * @param {string} [collapser] The xpath value used to find the internal tree element used to collapse a node.
     * @returns {Promise<void>} a promise that is fulfilled after the operation is performed.
     */
    collapse: function(nodePath, collapser){
        collapser = collapser === undefined ? null : collapser;
        return this._executeWithEvents(arguments, "Collapse",nodePath,collapser);
    },
    /**
     * Expands this tree based on the specified path.
     * If the xpath parameter is passed, the specified node in this tree is expanded with both the node path and XPath.
     * @param {string} [nodePath] The path to the tree node to expand. <br/>
     * Note: You can use the buildNodePath function to create the node path.
     * @param {string} [expander] The xpath value used to find the internal tree element used to expand and collapse a tree node.
     * @returns {Promise<void>} a promise that is fulfilled after the operation is performed.
     */
    expand: function(nodePath, expander){
        expander = expander === undefined ? null : expander;
        return this._executeWithEvents(arguments, "Expand",nodePath,expander);
    },
    /**
     * Builds the node path from the arguments passed.
     * Arguments can be either a string (the name of the node) or a number (the index of the node).<br/>
     * Example: If ("node1", 4) is passed, selects the 'node1' top level node item, and then selects the 4th node item in the opened nodes.
...
// SIG // wQNUg9GVVZNxu2v7/8rKTzpd/fvlQ6qp0iX4NIaTwpKg
...
// SIG // DjAMBgorBgEEAYI3AgEVMC8GCSqGSIb3DQEJBDEiBCA3
// SIG // qDUyRRH03mIhdbNv8TYx5cLNEc2qZGULH/Fs/tihMTAN
// SIG // BgkqhkiG9w0BAQEFAASCAQCEg2LTTKGFZGMoS5Yodmgn
// SIG // jbEUHpnwXgRI/d6POfiY6AFhd+i6S9T3jU1apig+FD3p
// SIG // ouxJ5GzB/q17Hur1ifQSoPjO0dREIdq3NKLZVV9aM+KZ
// SIG // nnTBhVS0GxQjfMnbEaIkM/lxwW5fFU/21r0aQPtMpWLn
// SIG // OYVD1dWP4ZQ8FGHtmQVjG3gHTpU5ux/OHBG7s7VdaJqE
// SIG // L7eKKr3lUnaXDWhN+VWHwRpokgbpVwwLlx0DOtP1dspp
// SIG // e+fkpMm7Wg9ZwA5T+ChBtCPagnx3kOWF5jojmFs8ptVz
// SIG // k4JMWWoOXCA2KZLm9TFZjAbeff88EOtRp8C+JZzy53Bu
// SIG // JhEuxr1nVpgboYICCzCCAgcGCSqGSIb3DQEJBjGCAfgw
...
// SIG // KoZIhvcNAQkFMQ8XDTE2MDUyNjE0MjYwNlowIwYJKoZI
// SIG // hvcNAQkEMRYEFF1xL6TwkFPKu/kjZ7K/DR0UWcIXMA0G
// SIG // CSqGSIb3DQEBAQUABIIBAHC3HC7lBMoj20bQ+1XVw51c
// SIG // lEK5+6mGimAAKXt0Q/iG0TWYDZ/ozdeufsFJB1qyzISs
// SIG // ez9Ff/DT3llgn7DM/gFQgNO5xLRv51BOIOweBqdZs6++
// SIG // 5PBz/eZti8mH2H+HBSdVRiPRBsPLqNd8Xwakh3BASY+f
// SIG // ptlXWJlTNLO23z1MuzJcksKS6QyBdBs7Cbp5jtGdEtHU
// SIG // Rjg/djYTCcXidQUtk5EDAcWCFKlDD+UqgkEoAats9BkH
// SIG // bLwiAz0EShXKU5++tR0G1mkA7KR3S9nAnWmAmzVeJ5rJ
// SIG // rnc/onNHFsP+Q0kAdJyIuANKxI+o8J2YYftjT9WsyP0a
// SIG // FdiB3vlrREI=
