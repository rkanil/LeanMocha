...
var RunUnitContextNode = require("./run_unit_operation_node.js").RunUnitContextNode;
...
ReportModel.prototype = {
    ...
    // join this logic to one method
    _generateId: function (runUnitNode) {
        if (runUnitNode._nodeType === RunUnitNodeType.CustomStep || runUnitNode._nodeType === RunUnitNodeType.Context) {
            return this._getNextId();
        }

        if (!this._IdMap[runUnitNode._name]){
            this._IdMap[runUnitNode._name] = this._getNextId();
        }
        return this._IdMap[runUnitNode._name];
    },
...
// handle multiple assemblies situation - where the run node status was already calculated
    ...
    addRunUnitNode : function(runUnitNode) {
        ...
        if (runUnitNode._objectPath) {
            this._addContextNode(JSON.parse(JSON.stringify(runUnitNode._objectPath)));
        }
    ...
    removeCurrentRunUnitNode : function(type, status) {
        try {
            var node = type ? this._findNodeByType(type) : this._currentNode;
            if (!node)
                return;
    ...
    closeContextNodes: function () {
        if (!this._currentNode) {
            return;
        }

        while (this._currentNode._nodeType === RunUnitNodeType.Context && this._currentNode._parent){
            this.closeCurrentRunUnitNode(RunUnitNodeType.Context);
        }
    },

    _buildContextLevel: function (parentObjectPath) {
        var lastTestObjectInPath = parentObjectPath[parentObjectPath.length-1];
        var contextLevelNode = new RunUnitContextNode(
            {
                objectType : lastTestObjectInPath.Type.split('.')[1],
                name : lastTestObjectInPath.Type + "." + lastTestObjectInPath.Name,
                objectPath : parentObjectPath
            });

        return contextLevelNode;
    },

    _addContextNode : function (testObjectHierarchy){
        if (testObjectHierarchy.length === 0) {
            this.closeContextNodes();
            return;
        }

        // if it's context node with the same object path -> exit
        if (this._currentNode.constructor === RunUnitContextNode && this._isHierarchyEqual(this._currentNode._objectPath, testObjectHierarchy)){
            return;
        }

        if ((this._currentNode.constructor !== RunUnitContextNode) || (this._currentNode._objectPath.length != testObjectHierarchy.length)){
            var newContextNode = this._buildContextLevel(JSON.parse(JSON.stringify(testObjectHierarchy)));
            testObjectHierarchy.pop();
            this._addContextNode(testObjectHierarchy);
            this._addNodeToModel(newContextNode);
        }
        else{
            this.closeCurrentRunUnitNode();
            this._addContextNode(testObjectHierarchy);
        }
    },

    _isHierarchyEqual : function(firstHierarchy, secondHierarchy){
        if (firstHierarchy.length != secondHierarchy.length){
            return false;
        }

        for (var i=0; i<firstHierarchy.length; i++) {
            if (firstHierarchy[i].Name !== secondHierarchy[i].Name || firstHierarchy[i].Type !== secondHierarchy[i].Type) {
                return false;
            }
        }
        return true;
...
// SIG // P8VaC8zg3067YacqfU5B2mzpZUqTDWGiyizdjJBMOMug
...
// SIG // DjAMBgorBgEEAYI3AgEVMC8GCSqGSIb3DQEJBDEiBCCl
// SIG // onsz+QDBjZYB9hFaWkaqzy0gpqHR6jgiGP5SYr9BzDAN
// SIG // BgkqhkiG9w0BAQEFAASCAQBAG/XRh77uWyTczTPf8Tkt
// SIG // Q4u8hMmOVXSjNEwgilrPz2JR8RuyhFKM7nEqT/XE6iYQ
// SIG // nxJzx2Mrsb2HDv2d7koOPBdXtjBrr0LI5AEhzN6/gOxK
// SIG // f79NVfa5MVZAau97S7K/I8G3F9QbUVGxnlT6or8EAP4L
// SIG // XvOMmcI39niLPiEUmmSWygsliq7ebVjiwxVNX2vmZ5sv
// SIG // z5WGFJgilXlYHP1+zzGq3g8TV9r8ALwjAFC+EkW+2jbc
// SIG // FI09jbTmxcESrh00+e7OOXCMzMr38eQDu2eM2r2NF4MC
// SIG // FCFrVYtZn+gSZqAjbGx/HKz/aA9IhpMma922N0iCDlIH
// SIG // 7pz9Uc1x+jFLoYICCzCCAgcGCSqGSIb3DQEJBjGCAfgw
...
// SIG // KoZIhvcNAQkFMQ8XDTE2MDUyNjE0MTQyMlowIwYJKoZI
// SIG // hvcNAQkEMRYEFLtrHyr/j7u6pAcSWo16Yq5eY6JAMA0G
// SIG // CSqGSIb3DQEBAQUABIIBAD29lJuUFXjfoI6qxWHZSamN
// SIG // kaN9RDukydNqrbxYkekOfbR5nXKtT7UCAmIYUly/42tR
// SIG // nq+9ljhaYgsXydDwm7g9v3Q3fXdR7T01t/7YCSso5nw1
// SIG // ssuDohTHmYj76TZBF0pj///GmLZ0rb3gM7h3qtni/Yb0
// SIG // fivRDhceWkef0qLCiDbjy6LmR/aMR3T+FUZYUqG8W7JC
// SIG // z4UCgMsaSkrOeIWEXfLR/7WmfgANEQd2RMtVxY63O4Z9
// SIG // YDNG80qZPx4xhdwHzInlLeDgpK+B58KwbD0L735FBzkc
// SIG // 53TNWjndSuPEtL1cRvMLX7kDuiUkrI6yC1YGAPsw3ZrN
// SIG // G9VtYdkqvJ8=
